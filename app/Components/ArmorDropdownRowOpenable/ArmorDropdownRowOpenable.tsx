...
export interface ArmorDropdownRowOpenableProps {
    title: string;
...
export default function ArmorDropdownRowOpenable({title, onPress, style, textStyle= localStyles.buttonText, disabled = false} : ArmorDropdownRowOpenableProps): JSX.Element {

    const isDarkMode = useColorScheme() === 'dark';
    const colors =  isDarkMode ? Colors['dark'] : Colors['light'];
    const styles = GlobalStyles['phone']

    //get distinct types from the armor.json
    const distinctTypes = Array.from(new Set(armor.map((item) => item.type)));
